...
// WASI-CHANGE: We'll just timeout through ctest
#ifndef __wasi__
/* Timeout thread */
void * timer(void * arg)
{
 ...
 */
	...
	for (scenar=0; scenar < NSCENAR; scenar++)
	{
		...
		for (ch=0; ch < NTHREADS; ch++)
		{
			...
			{
				#ifdef __wasi__
				FAILED("WASI does not support forking, which is required for this test scenario");
				#else
				p_child[ch]=fork();
				if (p_child[ch] == -1)
				{
		...
		// WASI-CHANGE: We will just timeout through ctest, so we don't need to cancel the timer thread
		#ifndef __wasi__
